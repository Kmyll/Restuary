			...
			<React.Fragment>
				...
				<section className='ExplorePage'>
					<div className='container exploreList'>
						...
						{
							posts.length ? posts.filter(searchingFor(term)).map((post) => (
								<ul>
									<li key={post.id}>
										<img className='Explore_list_flag' src={post.flag} /> <p>{post.name}</p>
										<span className='Explore_List_Arrow'>
											...
											>
												<Modal.Header>
													<h2 className='modalTitle'>
														<FaPlane className='ModalPlane' /> {post.name}{' '}
													</h2>
												</Modal.Header>
												<Modal.Content image>
													<Image
													...
													<Modal.Description>
														<ul className='modalLi'>
															<li>
																<span>Capital city:</span> {post.capital}
															</li>
															<li>
																<span>Continent:</span> {post.region}
															</li>
															<li>
																<span>Population:</span> {post.population}
															</li>
															<li>
																<span>Timezone:</span> {post.timezones}
															</li>
															<li>
																<span>Currency:</span> {post.currencies[0].name} - (
											...
											<Link className='homeLink' to={`${ROUTES.EXPLORE}/${post.name}`} />
										</span>
									</li>
								</ul>
							)) :
